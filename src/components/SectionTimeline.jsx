...
const SectionTimeline = () => (
  <section id="experience" className="relative section-spacing bg-transparent">
    <div className="mx-auto max-w-6xl px-6 py-20 md:px-12">
      ...
      <div className="relative mt-12 ml-2 pl-6 before:absolute before:left-3 before:top-0 before:h-full before:w-1 before:bg-primary/30">
        {timelineRows.map((row, idx) => (
          <motion.div key={row.year} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: idx * 0.1 }} className="relative mb-10 pl-6">
            <div className="absolute left-0 top-1 h-3 w-3 rounded-full bg-primary shadow-glow" />
            <div className="rounded-2xl border border-primary/20 bg-slate-900/40 p-5 shadow-glow backdrop-blur-xl transition hover:-translate-y-1">
              <p className="text-sm font-semibold text-primary">{row.year}</p>
              <h3 className="mt-1 text-xl font-semibold text-white">{row.title}</h3>
              <p className="text-sm text-slate-400">{row.subtitle}</p>
